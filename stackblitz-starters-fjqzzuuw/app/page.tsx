...
import { ListCollapse, Grid, GripVertical, Settings, Sun, Moon, Zap } from 'lucide-react';
...
export default function HomePage() {
  ...
  const [editingTeam, setEditingTeam] = useState<{ team: Team, potIndex: number, teamIndex: number } | null>(null);
  const [tempStats, setTempStats] = useState<Team['stats'] | null>(null);
  const [tempName, setTempName] = useState<string>('');
  
  const [draggedItem, setDraggedItem] = useState<{ potIndex: number; teamIndex: number } | null>(null);
  ...
  const handleOpenEditModal = (team: Team, potIndex: number, teamIndex: number) => {
  ...
  const onDrop = (targetPotIndex: number) => {
    ...
    const { potIndex: sourcePotIndex, teamIndex: sourceTeamIndex } = draggedItem;
        ...
        <Dialog open={!!editingTeam} onOpenChange={() => setEditingTeam(null)}>
          <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Ajusta el nombre y las estadísticas del equipo. Los valores deben estar entre 0 y 10.
                </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input id="level" type="number" value={tempStats.level} onChange={(e) => handleStatChange('level', e.target.value)} className="col-span-3" min="0" max="10" step="0.1"/>
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="goalCapacity" className="text-right">Cap. Gol</Label>
                    <Input id="goalCapacity" type="number" value={tempStats.goalCapacity} onChange={(e) => handleStatChange('goalCapacity', e.target.value)} className="col-span-3" min="0" max="10" step="0.1"/>
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="defenseCapacity" className="text-right">Cap. Defensa</Label>
                    <Input id="defenseCapacity" type="number" value={tempStats.defenseCapacity} onChange={(e) => handleStatChange('defenseCapacity', e.target.value)} className="col-span-3" min="0" max="10" step="0.1"/>
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="hierarchy" className="text-right">Jerarquía</Label>
                    <Input id="hierarchy" type="number" value={tempStats.hierarchy} onChange={(e) => handleStatChange('hierarchy', e.target.value)} className="col-span-3" min="0" max="10" step="0.1"/>
                </div>
            </div>
            <DialogFooter>
                <DialogClose asChild>
            ...
            {pots.map((pot, potIndex) => (
              <Card 
                ...
                className={cn(
                  ...
                  dragOverPot === potIndex && "border-primary ring-2 ring-primary"
                )}
              ...
              >
                <CardHeader>
                  <CardTitle className="flex justify-between items-center">
                    ...
                    <Badge variant={pot.length === 16 ? "default" : "secondary"}>{pot.length} / 16</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    {pot.map((team, teamIndex) => (
                      <li 
                        ...
                        className={cn(
                            ...
                            draggedItem?.potIndex === potIndex && draggedItem?.teamIndex === teamIndex && "opacity-50"
                        )}
                      >
                        ...
                        <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100" onClick={() => handleOpenEditModal(team, potIndex, teamIndex)}>
                            <Settings className="h-4 w-4"/>
                        </Button>
                      </li>
                    ))}
                    {pot.length === 0 && (
                      <li className="text-sm text-center text-muted-foreground p-2">Bombo vacío</li>
                    )}
                  </ul>
                </CardContent>
              </Card>
         ...
         <section id="settings">
          <Card className="max-w-md mx-auto">
            ...
            <CardContent className="space-y-6">
              <div>
                <label htmlFor="surprise" className="block text-sm font-medium mb-2">Nivel de Sorpresa: <span className="font-bold text-accent">{surpriseLevel}</span></label>
                <p className="text-xs text-muted-foreground mb-4">A mayor nivel, resultados más impredecibles.</p>
                <Slider
                  id="surprise"
                  min={0}
                  max={10}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <Card className="hover:border-primary/80 hover:shadow-lg hover:shadow-primary/20 transition-all duration-300">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                Sorteo clásico con 16 grupos de 4 equipos. Los dos mejores avanzan a la fase eliminatoria.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button className="w-full" onClick={() => handleStartTournament('/groups')}>
          ...
          <Card className="hover:border-accent/80 hover:shadow-lg hover:shadow-accent/20 transition-all duration-300">
             <CardHeader className="text-center">
                ...
                <CardDescription>
                    Torneo con fases preliminares. Los equipos de bombos inferiores luchan por un puesto en la fase final.
                </CardDescription>
            </CardHeader>
            <CardContent>
              <Button className="w-full" variant="outline" onClick={() => handleStartTournament('/keys')}>
